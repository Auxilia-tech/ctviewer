...
parser.add_argument("-i", "--images", default="/Users/lounes/Workspace/temp_processed/images", type=str, metavar="PATH", help="path to images folder")
parser.add_argument("-a", "--annotations", default="/Users/lounes/Workspace/temp_processed/annotation.json", type=str, metavar="PATH",  help="path to annotations json file")
...
def draw_bboxes(draw, objects, labels, obj_categories, ignore, width, label_size):
    ...
    scores = [
    [
        "  " + str(float("%.2f" % (obj["score"])))
    ]
    for obj in objects
    ]
    # Draw bboxes
    for i, (c, b, d) in enumerate(zip(obj_categories, bboxes, scores)):
        if i not in ignore:
